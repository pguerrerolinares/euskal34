...
cancelar ni umbral que ajustar a ojo.
  ...
  La diagonal sale 10.2327 -- ES la masa. p = m*v con m ~ 10 (al reves que en
  solve.py, o que en la version vieja de este mismo fichero, que al despejar
  "v entre p" sacaban 0.0996, la inversa de la masa). De los dos numeros, este
  es el que tiene sentido fisico.
  ...
  M[i,i] de ~10 en sigmas se cuela ahi tal cual; solo 1 celda es ruido de
  verdad). Nota sobre el control de fondo: aqui no se cancela la diagonal en
  ningun paso (ver diferencia con solve.py mas abajo), asi que las 9 filas de
  fondo que caen dentro del rango de COLS (los huecos 114-117, 127-130, 140
  entre bandas de texto) chocan con su propio M[i,i] al mirar las columnas de
  la ventana -- 9 celdas puntuales a 125-128 sigma que son la masa, no ruido.
  El control las excluye de una en una por su mascara diagonal (fila==columna),
  no descartando las 9 filas enteras: eso conservaria las otras 234 celdas de
  esas mismas filas (5940 - 9 = 5931) en vez de tirar 243 celdas de control por
  9 valores identificados. Medir sobre la maxima poblacion valida, no sobre la
  que da el numero comodo, es justo el tema de este writeup.
  ...
  diagonal en una masa interpretable (m~10) en vez de un escalar que depende
  de la escala arbitraria de 16 bits.
...
def reconstruye_m():
    ...
    fuera = ~np.eye(256, dtype=bool)          # la diagonal es la fisica (m~10), no el mensaje
...
def main():
    ...
    print("diagonal (la fisica del reto, es la masa): %.6f\n" % alpha)
    ...
    # celdas puntuales a 125-128 sigma que son la masa, no ruido. Se excluyen
